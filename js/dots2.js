...
function comprobarRecord(){ //comprobar si la partida está entre las diez mejores
    $.ajax({
        url: "comprobar.php",
        data: {
            puntos : puntoNumero,
        ...
        success: function(resp){
            ...
            var escrito = 0; //usaremos para posicionar la partida actual entre las mejores
            if(resp == 0){ //no está entre las mejores
                $('#gameover').modal('show');
            }
            else if(resp == 1){ //no hay partidas. Es la primera.
                var fecha = new Date();
                $('#records > tbody').append('<tr class="nuevo-record"><td>' + pos + '</td><td ><input type="text" id="tunombre" placeholder="Escribe tu nombre..." /></td><td>' + puntoNumero + '</td><td>' + imprimirFecha() + '</td></tr>' );
                if(tipoPartida == "m"){
                    $('#tipop').html('Movimientos');
                }
                else{
                    $('#tipop').html('Contrareloj');
                }
                $('#topscores').modal('show');
            }
            else{ // está entre las mejores.
                
                $('#records > tbody').empty(); //borramos contenido del tbody, por si está de una partida anterior.
                respJ = JSON.parse(resp);
                $.each(respJ, function(index, element){
                    if(escrito === 0 && puntoNumero > element.puntos){
                        $('#records > tbody').append('<tr class="nuevo-record"><td>' 
                                + pos + '</td><td ><input type="text" id="tunombre" placeholder="Escribe tu nombre..." /></td><td>' 
                                + puntoNumero + '</td><td>' + imprimirFecha()  + '</td></tr>' );
                        pos++;
                        escrito = 1;
                    }
                    
                    
                    $('#records > tbody').append('<tr><td>' + pos + '</td><td>' + element.nombre + '</td><td>' + element.puntos + '</td><td>' + formatoFecha(element.fecha) + '</td></tr>' );
                    pos++;
                    if(pos === 10){
                        return false; // Hace un break del bucle each, para que sólo muestre 10 registros
                    }
                });    
                
                if(escrito === 0 && pos < 10){ //Caso de que no haya diez registros, la partida actual está entre las mejores
                        console.log("no hay 10");
                        $('#records > tbody').append('<tr class="nuevo-record"><td>' 
                                + pos + '</td><td ><input type="text" id="tunombre" placeholder="Escribe tu nombre..." /></td><td>' 
                                + puntoNumero + '</td><td>' + imprimirFecha()  + '</td></tr>' );
                        pos++;
...
function imprimirFecha(){
    ...
    if(fecha.getMonth() < 10){
        fechaFinal += "0" + fecha.getMonth();
    }else{
        fechaFinal += fecha.getMonth();
    }
    
    fechaFinal += "-" + fecha.getFullYear();
    return fechaFinal;
...
function otraPartida()
{
	...
	document.getElementById("oscuro").style.visibility = "hidden";
